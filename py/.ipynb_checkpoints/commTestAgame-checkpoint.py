...
#if running in parallel... do this here
import pp
ppservers=()
ncpus=4            
...
def TestCalcNumpy(num_samples,num_methyls,ack,r_cross,r_diff,tau,ii_xrd,jj_xrd,jj_nmr,init_intensities):
...
class mr_noesy():
    def __init__(self,peak_file_dir,peak_list_dir,pdb_file_dir,macro=False, ax=True, xx=True, cx=True, ca=True, sparse=True, sym=True, irreducible=False, num=False):
        ...
        ##PARALLEL###
        self.num_samples=1000  #SET BY AJB: WAS 2000
        ...
        self.xrd_indices,self.nmr_indices=xrd_indices,nmr_indices #save these as globals
        ...
        print 'num_samples=2000'
        for i in numpy.arange(10):
            self.CalcModel()
        self.num_samples=4000
        print 'num_samples=4000'
        for i in numpy.arange(10):
            self.CalcModel()
        self.num_samples=8000
        print 'num_samples=8000'
        for i in numpy.arange(10):
            self.CalcModel()
...
#         self.init_intensities=x[:,0]
    ...
    def TestCalcOriginal(self,r_cross,r_auto):
        #ORIGINAL
        ...
        return sim_matrix[self.ii_xrd,self.jj_xrd]*self.init_intensities[self.jj_nmr,self.jj_nmr]
    ...
    def CalcModel(self):
        ...
        now1=datetime.now()
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=self.TestCalcOriginal(r_cross,r_auto)         #MATCH SIMULATED XRD DATA ONTO NMR INDICES:
        print 'original:',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)
        compare=copy.deepcopy(self.xrd_peaks)
        ...
        if(self.parallel_flg=='y'): #use numpy function outside class
            ...
            jobs = [(i, job_server.submit(TestCalcNumpy,(self.num_samples,self.num_methyls,self.PackPar[i],r_cross,r_diff,self.tau,self.ii_xrd,self.jj_xrd,self.jj_nmr,self.init_intensities),(),("numpy","scipy.linalg",))) for i in range(self.ncpus)]
            ...
            now1=datetime.now()
            self.xrd_peaks[self.ii_nmr,self.jj_nmr]=TestCalcNumpy(self.num_samples,self.num_methyls,self.Package,r_cross,r_diff,self.tau,self.ii_xrd,self.jj_xrd,self.jj_nmr,self.init_intensities)
            print 'numpy2:   ',datetime.now()-now1
            print 'checksum:',numpy.sum(self.xrd_peaks),numpy.average(self.xrd_peaks-compare)/numpy.average(self.xrd_peaks)*100.
