...
#swap diagonal from auto onto cross, subtract diagonal auto from off diagonal
#do this once.
@njit(nopython=True,nogil=True,fastmath=True)
def RateSwap(r_cross,r_auto,r_diff):
    for i in range(len(r_auto)): #first, subtract diagonal from auto, and add it to cross.
        for j in range(len(r_auto)):
            r_diff[i,j]=r_auto[i,j]-r_auto[i,i]
        r_cross[i,i]+=r_auto[i,i]


#numba function for calculating final rate matrix
@njit(nopython=True,nogil=True,fastmath=True)
def TestCalc(num_samples,num_methyls,ack,r_cross,r_diff,tau,xrd_indices,nmr_indices,init_intensities):
    result_matrix=numpy.zeros((num_methyls,num_methyls)) #this needs to be full size            
    for p in range(len(ack)): #loop over sampling schedules
        ll=ack[p]
        rate_matrix=numpy.zeros((len(ll),len(ll)))
        expy=numpy.zeros((len(ll),len(ll)))
        for i in range(len(ll)):
            for j in range(len(ll)):
                rate_matrix[i,j]=r_cross[ll[i],ll[j]] #diagonal already included
        for i in range(len(ll)):
            for j in range(len(ll)):
                rate_matrix[i,i]+=r_diff[ll[i],ll[j]] #+r_auto[i,j]  #add sum of off diagonals
        rate_matrix*=-tau
        with objmode(expy='float64[:,:]'):
            expy=scipy.linalg.expm(rate_matrix)#update matrix
        #expy=scipy.linalg.expm(rate_matrix)#update matrix
        for i in range(len(ll)):
            for j in range(len(ll)):
                result_matrix[ll[i],ll[j]]+=expy[i,j]#update matrix
    result_matrix/=num_samples
    xrd=numpy.zeros((len(xrd_indices),len(xrd_indices)))
    for i in range(len(nmr_indices)):
        for j in range(len(nmr_indices)):
            xrd[i,j]=result_matrix[xrd_indices[i],xrd_indices[j]]*init_intensities[nmr_indices[j],nmr_indices[j]]
    return xrd

#numba function for calculating final rate matrix
#@njit(nopython=True,nogil=True,fastmath=True)
def TestCalcNumpy(num_samples,num_methyls,ack,r_cross,r_diff,tau,ii_xrd,jj_xrd,jj_nmr,init_intensities,diag):
    result_matrix=numpy.zeros((num_methyls,num_methyls)) #this needs to be full size            
    from scipy.linalg import expm
    for ii,jj in ack:
        rate_matrix=r_cross[ii,jj] #diagonal already included
        rate_matrix[diag]+=numpy.sum(r_diff[ii,jj],axis=1) #+r_auto[i,j]  #add sum of off diagonals
        result_matrix[ii,jj]+=scipy.linalg.expm(-rate_matrix*tau)#update matrix
    result_matrix/=num_samples
    #print result_matrix
    return result_matrix[ii_xrd,jj_xrd]*init_intensities[jj_nmr,jj_nmr]
...
class mr_noesy():
    def __init__(self,peak_file_dir,peak_list_dir,pdb_file_dir,macro=False, ax=True, xx=True, cx=True, ca=True, sparse=True, sym=True, irreducible=False, num=False):
        ...
        ##PARALLEL###
        self.num_samples=100000  #SET BY AJB: WAS 2000



        ##############################
        #things we can pre-compute:
        ...
        #xx,yy=numpy.meshgrid(x,y,indexing='ij') #not needed I think?
        ...
        for rep in numpy.arange(self.num_samples): #calculate sampling sechdules.
            ...
            self.Pock.append(index_list) #numba method
        self.Pock=numpy.array(self.Pock)
        self.diag=numpy.diag_indices(len(index_list)) #numpy method
        ##############################


        #################################
        #chunk jobs for parallelisation
        self.ncpus=4            
        self.PackPar=[]
        for j in range(self.ncpus):
            self.PackPar.append([])
        for i in range(len(self.Package)):
            self.PackPar[i%self.ncpus].append(self.Package[i])
        #################################
        ...
        self.xrd_indices,self.nmr_indices=xrd_indices,nmr_indices
...
#         self.init_intensities=x[:,0]
    ...
    def TestCalcNumpy(self,ack,r_cross,r_diff):
        result_matrix=numpy.zeros((self.num_methyls,self.num_methyls)) #this needs to be full size            
        for ii,jj in ack:
            rate_matrix=r_cross[ii,jj] #diagonal already included
            rate_matrix[self.diag]+=numpy.sum(r_diff[ii,jj],axis=1) #+r_auto[i,j]  #add sum of off diagonals
            result_matrix[ii,jj]+=scipy.linalg.expm(-rate_matrix*self.tau)#update matrix
        result_matrix/=self.num_samples
        #print result_matrix
        return result_matrix[self.ii_xrd,self.jj_xrd]*self.init_intensities[self.jj_nmr,self.jj_nmr]        
    ...
    def CalcModel(self):
        ...
        self.num_methyls=r_cross.shape[0] #we know this in advance I think?

        now1=datetime.now()

        #ORIGINAL
        result_matrix=numpy.zeros((self.num_methyls,self.num_methyls,self.num_samples))
        #Calculate intensities by repeating for 2000 combinations:
        for rep in numpy.arange(self.num_samples): #calculate sampling sechdules.
            list_methyls=self.methyl_list_generator(self.residues)
            #print list_methyls
            ...
            exp_matrix[:,:]=expm(-rate_matrix[:,:]*self.tau) ##we might not need the xx,yy index
            result_matrix[ii,jj,rep]=exp_matrix[xx,yy]#don't need to keep the index 'rep' in memory: 
            #result_matrix[ii,jj]+=exp_matrix[xx,yy]#don't need to keep the index 'rep' in memory: 
        #print result_matrix
        #sim_matrix/=self.num_samples #complete the average
        ...
        print 'original:',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)
    

        r_diff=numpy.zeros((len(r_auto[0]),len(r_auto[0])))  
        RateSwap(r_cross,r_auto,r_diff) #swap diagonal auto rate onto Rcross.
        #r_diff=r_auto-r_auto[self.diag]#subtract off the diagonal (same as r_auto[ii,jj]-r_auto[ii,ii]?)
        #r_crossC[self.diag]+=r_auto[self.diag] #do this once
        ...
        for ii,jj in self.Package:
            rate_matrix=r_cross[ii,jj] #diagonal already included
            rate_matrix[self.diag]+=numpy.sum(r_diff[ii,jj],axis=1) #+r_auto[i,j]  #add sum of off diagonals
            result_matrix[ii,jj]+=expm(-rate_matrix*self.tau)#update matrix
        result_matrix/=(self.num_samples*1.) #complete average
        #MATCH SIMULATED XRD DATA ONTO NMR INDICES:
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=result_matrix[self.ii_xrd,self.jj_xrd]*self.init_intensities[self.jj_nmr,self.jj_nmr]


        print 'numpy:   ',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)


        now1=datetime.now()
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=TestCalcNumpy(self.num_samples,self.num_methyls,self.Package,r_cross,r_diff,self.tau,self.ii_xrd,self.jj_xrd,self.jj_nmr,self.init_intensities,self.diag)
        print 'numpy2:   ',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)


        #internal class numpy
        now1=datetime.now()
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=self.TestCalcNumpy(self.Package,r_cross,r_diff)
        print 'numpy3:   ',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)


        #compiling version of numba(first call will be slow)
        now1=datetime.now()
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=TestCalc(self.num_samples,self.num_methyls,self.Pock,r_cross,r_diff,self.tau,self.xrd_indices,self.nmr_indices,self.init_intensities)
        print 'numba(compile):',datetime.now()-now1
        print 'checksum: ',numpy.sum(self.xrd_peaks)

        #compiled timing of numba
        now1=datetime.now()
        self.xrd_peaks[self.ii_nmr,self.jj_nmr]=TestCalc(self.num_samples,self.num_methyls,self.Pock,r_cross,r_diff,self.tau,self.xrd_indices,self.nmr_indices,self.init_intensities)
        print 'numba(run):',datetime.now()-now1
        print 'checksum:',numpy.sum(self.xrd_peaks)
        ...
        if(self.parallel_flg=='y'): #use numpy function outside class
            now1=datetime.now()
            print 'running in parallel'
            import pp
            ppservers=()
            job_server = pp.Server(ncpus = self.ncpus, ppservers = ppservers, restart=True)
            jobs = [(i, job_server.submit(TestCalcNumpy,(self.num_samples,self.num_methyls,self.PackPar[i],r_cross,r_diff,self.tau,self.ii_xrd,self.jj_xrd,self.jj_nmr,self.init_intensities,self.diag),(),("numpy","scipy",))) for i in range(self.ncpus)]
            self.xrd_peaks[self.ii_nmr,self.jj_nmr]=0.0 #reset array
            for i,job in jobs:
                self.xrd_peaks[self.ii_nmr,self.jj_nmr]+=job()
                
            job_server.print_stats()
            import time
            time.sleep(2)
            job_server.destroy()
            print 'numpy(parrun):',datetime.now()-now1
            print 'checksum:',numpy.sum(self.xrd_peaks)
        else:
            now1=datetime.now()
            self.xrd_peaks[self.ii_nmr,self.jj_nmr]=TestCalcNumpy(self.num_samples,self.num_methyls,self.Package,r_cross,r_diff,self.tau,self.ii_xrd,self.jj_xrd,self.jj_nmr,self.init_intensities,self.diag)
        ...
        if(self.parallel_flg=='y'):#use numpy function inside class
            now1=datetime.now()
            print 'running in parallel2'
            import pp
            ppservers=()
            job_server = pp.Server(ncpus = self.ncpus, ppservers = ppservers, restart=True)
            jobs = [(i, job_server.submit(self.TestCalcNumpy,(self.PackPar[i],r_cross,r_diff),(),("numpy","scipy",))) for i in range(self.ncpus)]
            self.xrd_peaks[self.ii_nmr,self.jj_nmr]=0.0 #reset array
            for i,job in jobs:
                self.xrd_peaks[self.ii_nmr,self.jj_nmr]+=job()
